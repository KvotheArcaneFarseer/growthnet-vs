...
This project uses a dual-agent workflow combining Claude Code and Codex.

## Workflow Overview

1. **Architecture (Claude)**
   - Define tumor geometry (lollipop model)
   - Establish coordinate systems and constraints
   - Plan deterministic behavior

2. **Implementation (Codex)**
   - Generate or modify code
   - Apply changes across files
   - Handle debugging and iteration

3. **Validation Loop**
   - Claude analyzes outputs and metrics
   - Identifies issues such as:
     - incorrect geometry
     - growth direction errors
     - flawed validation logic

4. **Iteration**
   - Claude refines design decisions
   - Codex updates implementation

## Design Philosophy

- Claude = system-level reasoning  
- Codex = execution engine  
- Separation improves reliability and scalability  

## Outcome

This workflow enabled:
- Rapid development of synthetic tumor geometry
- Clean architecture decisions
- Scalable batch pipeline (Rivanna HPC)
